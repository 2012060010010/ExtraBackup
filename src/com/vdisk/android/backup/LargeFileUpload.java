...
public class LargeFileUpload extends AsyncTask<Void, Long, Boolean> {
	...
	private VDiskAPI<?> mApi;//微盘API类型
	private String mPath;   //微盘路径
	private File mFile;     //微盘文件名

	private long mFileLen;//文件长度
	private Context mContext;
	private final ProgressDialog mDialog;

	private String mErrorMsg;
	private String mSrcPath;    //源文件路径
	...
	public LargeFileUpload(Context context, VDiskAPI<?> api, String VDiskPath,
		...
		mDialog.setButton(DialogInterface.BUTTON_NEGATIVE,mContext.getString(R.string.be_cancel),
				new OnClickListener() {
	...
	protected Boolean doInBackground(Void... params) {
		try {
			...
			String desPath = mPath + mFile.getName();    //微盘上创建的文件路径名
			...
			mApi.putLargeFileOverwriteRequest(mSrcPath, desPath,
					mFile.length(), handler);//将大文件上传到云端
	...
	@Override
	protected void onProgressUpdate(Long... progress) {//进度条
